...
function Interface(hydro) {
  this.hydro = hydro;
  this.ctx = new Frame(hydro.topLevel);
  this.stack = [this.ctx];
}
...
_.forEach(['createTest', 'createSuite'], function(method) {
...
Interface.prototype.addTest = function() {
  ...
  _.forEach(this.ctx.beforeNext, before);
  this.ctx.beforeNext.length = 0;

  _.forEach(this.ctx.afterNext, after);
