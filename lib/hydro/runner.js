...
Runner.prototype.run = function(emitter, fn) {
  var self = this;
  var suites = this.suites.slice(0);
  var suite = null;

  function next() {
    if (suite = suites.shift()) return suite.run(emitter, next);
    emitter.emit('post:all', self, function() { fn(self); });
  }

  emitter.emit('pre:all', this, next);
};
